#!/usr/bin/env bash
# Creates PRs to rancher/rancher branches with updated rancher-assets image.
#
# Required env vars:
...
# Filter branches based on TAG's minor version
MINOR_VERSION=$(get_tag_minor_version "$TAG")
FILTERED_BRANCHES=$(filter_branches_for_tag "$TAG")
read -ra TARGET_BRANCHES <<< "$FILTERED_BRANCHES"

summary ""
summary "## Processing branches for tag $TAG (v$MINOR_VERSION)"
summary "Target branches: ${TARGET_BRANCHES[*]}"
...
for TARGET_BRANCH in "${TARGET_BRANCHES[@]}"; do
  ...
  # Convert release/v2.16 -> v2.16, main -> main
  TARGET_SUFFIX="${TARGET_BRANCH#release/}"
  BRANCH_NAME="bot/rancher-assets-${TAG}-${TARGET_SUFFIX}-$(date +%s)"
  ...
  # Commit changes
  COMMIT_MSG="Update rancher-assets to ${TAG}
...
Created-by: rancher-assets-release-integration"
  ...
  PR_BODY="## Summary
Update rancher-assets image to [\`${TAG}\`](https://github.com/${SOURCE_REPO}/releases/tag/${TAG})
